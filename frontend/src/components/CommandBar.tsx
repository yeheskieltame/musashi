  ...
  return (
    <form onSubmit={handleSubmit} className="flex flex-col sm:flex-row items-center gap-3 w-full">
      <div className="flex-1 relative w-full group">
        <div className="absolute inset-0 bg-amber-500/10 blur-xl opacity-0 group-hover:opacity-100 transition-opacity pointer-events-none rounded-xl" />
        <input
          type="text"
          value={input}
          onChange={(e) => setInput(e.target.value)}
          placeholder="Enter token address (0x...) or search by name..."
          className="relative w-full bg-white/5 border border-white/10 rounded-xl px-5 py-3.5 text-sm text-white placeholder-slate-500 outline-none focus:border-amber-500/50 focus:bg-white/10 transition-all font-mono shadow-[inset_0_2px_4px_rgba(0,0,0,0.3)]"
        />
        {loading && (
      ...
      <div className="flex w-full sm:w-auto gap-3">
        <select
          ...
          className="bg-white/5 border border-white/10 rounded-xl px-4 py-3.5 text-sm text-white outline-none cursor-pointer hover:bg-white/10 transition-colors focus:border-amber-500/50 appearance-none pr-8 relative"
        ...
        <button
          ...
          className="flex-1 sm:flex-none relative group overflow-hidden rounded-xl bg-amber-600 border border-amber-500/50 px-8 py-3.5 text-sm font-medium text-white shadow-[0_0_20px_rgba(217,119,6,0.4)] hover:shadow-[0_0_30px_rgba(217,119,6,0.6)] hover:bg-amber-500 transition-all disabled:opacity-50 disabled:shadow-none cursor-pointer whitespace-nowrap"
        >
